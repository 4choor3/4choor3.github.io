...
(function () {
  ...
  function loadSakana() {
    ...
    s.onload = function () {
      try {
        ...
        // 右移 34px（2026-09-14 站主要求）：right:0 → -34px，
        // 角色画布 180px 从容器左上起，右移后画布右缘仍距屏幕右缘 26px，不会切边
        mountEl.style.cssText =
          "position:fixed;right:-34px;bottom:0;z-index:9998;width:240px;height:200px;";
  ...
  // 手机端不加载挂件（2026-09-15 站主要求）：容器 240×200 在窄屏占比过大，
  // 且厂商库 87KB + 角色图对手机是纯负担。断点 768px 与站点其它移动端规则一致。
  // CSS 侧另有 @media 兜底隐藏（覆盖"先在宽屏加载、再把窗口缩窄"的场景）。
  // 若初始是窄屏、之后把窗口拉宽到桌面尺寸，再按需加载一次。
  var MOBILE_MAX = 768;
